...
- Task: `Sync raw source changes for sf5: federation-impact, methodology, routing-impact, runtime`
- Objects: 9
- Relations: 20
...
- `policy.sf5.validation-hardening` (0.69): Component/recipe changes need validation hardening, working-set coverage and release gate evidence.
- `gate.sf5.release-readiness` (0.69): Blocks completion until component/loader changes have working-set, validation and release-gate evidence.
- `capability.sf5.page-recipes` (0.54): Route work to page recipes and scenario playbooks for landing, dashboard, catalog, auth, checkout and profile flows.
- `capability.sf5.tailwind-conversion` (0.54): Convert Tailwind/UI source into SF5 components with working-set coverage and validation hardening.
- `skill.sf5.core` (0.51): Owns SIMAI Framework 5 frontend-first workflows: loader, utilities, components, smart-components, recipes and staged backend planning.
- `policy.sf5.frontend-first-boundary` (0.51): Keep SF5 frontend-first scope explicit and escalate staged backend work to the owning platform skill.
...
- `capability.sf5.smart-components` (0.36): Design and validate smart components, catalog entries, templates and smart hints.
