...
REQUIRED_ARTIFACTS = {
    "semantic": Path(
        f"graph/generated/growgraph/semantic-preservation/"
        f"{SKILL}-repo-local-semantic-review-{RUN_DATE}/semantic-preservation-verdict.json"
    ),
    "effectiveness": Path(
        f"graph/generated/growgraph/effectiveness-reports/"
        f"{SKILL}-repo-local-effectiveness-{RUN_DATE}/effectiveness-report.json"
    ),
    "adoption": Path(
        f"graph/generated/growgraph/adoption-reports/"
        f"{SKILL}-repo-local-adoption-gga9-active-gate-{RUN_DATE}/growgraph-adoption-report.json"
    ),
    "federation": Path(
        f"graph/federation/exports/{SKILL}-repo-local-federation-export-{RUN_DATE}/federation-export.json"
    ),
    "integration": Path(
        f"graph/generated/growgraph/integration-reviews/"
        f"{SKILL}-repo-local-integration-review-active-gate-{RUN_DATE}/growgraph-integration-review.json"
    ),
}
