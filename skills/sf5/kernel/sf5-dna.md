...
- When SF5 output is adapted into Blade or similar PHP templates, keep templates presentation-only: markup plus ordinary output, includes, conditionals and loops. CSS/JS belongs in asset files connected through the owner asset pipeline, and data preparation, formatting, mapping, helper functions, service calls and other PHP computations belong in controllers, view models, presenters, components, services or runtime classes. Do not generate inline `<style>`, `<script>`, `@php`, raw PHP functions or calculation blocks in templates except as a documented legacy exception with owner, reason, cleanup trigger and evidence.
